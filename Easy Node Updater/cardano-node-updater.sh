...
CURRENT=$(cardano-node --version | grep -o -E '[0-9]+\.[0-9]+\.[0-9]+' | head -1) && \
...
echo "" && \
echo "An error occured when attempting to check cardano-node version. Is the PATH environment variable set?" && \
echo ""
else
cd $SCRIPTPATH && \
mkdir $LATEST && \
...
echo "Done!" && \
echo "" && \
NEW=$(cardano-node --version | grep -o -E '[0-9]+\.[0-9]+\.[0-9]+' | head -1) && \
TIME=$(date) && \
cd $SCRIPTPATH && \
